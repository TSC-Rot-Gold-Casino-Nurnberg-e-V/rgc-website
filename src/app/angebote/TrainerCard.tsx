  ...
  <div className="flex gap-4 max-sm:mx-auto max-sm:max-w-sm max-sm:flex-col sm:gap-8">
    ...
    <section className="space-y-4">
      <div className="space-y-2">
        <h3 className="pt-2 text-2xl font-semibold sm:text-3xl">{name}</h3>
        {lizenzen.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {lizenzen.map((lizenz) => (
              <div
                key={lizenz.id}
                className="rounded-full border border-secondary-950 px-4 py-1 text-xs text-secondary-950 sm:text-sm"
              >
                {lizenz.name}
              </div>
            ))}
